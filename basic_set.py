...
def define_set():
    ...
    empty = {} # 주의 : 빈 SET이 아니라 빈 사전
    print(empty, type(empty))

    # 순서가 없고, 인덱스도 없고, 슬라이싱도 안되고
...
def set_methods():
    ...
    evens.discard(4)
    print("짝수집합:", evens)
    evens.discard(4) # discard -> 없는 요소를 삭제해도 에러 발생 X
...
def set_oper():
    ...
    print("홀수집합이 전체집합의 부분집합?", odds.issuperset(numbers))
