...
def transform_methods():
    ...
    print("SWAPCASE:", s.swapcase()) # 대문자 <-> 소문자 번환
    print("CAPITALIZE:", s.capitalize()) #문장 첫 글자 대문자로
    print("TITLE:", s.title()) #기사 제목 형태로 : 각 단어의 첫글자를 대문자로

    print("원본:", s) # str 객체 immutable -> 원본 변경되지 않음
...
def search_methods():
    ...
    print("2nd Find:", s.find("Like", 6)) # 6번 인덱스 이후에 Like의 인덱스
    print("3nd Find:", s.find("Like", 20)) # 검색 결과가 없으면 -1 변환

    print("1st Index:", s.index("Like")) # 문자열 내에 Like의 인덱스
    print("2nd Index:", s.index("Like", 6)) # 6번 인덱스 이후 Like의 인덱스
    #print("3nd Index:", s.index("Like", 21)) # 검색 결과가 없으면 ValueError
    # 에러 발생시키는 메서드 사용시에는 미리 체크(방어코드)
    ...
    # rindex는 검색 결과 없을 떄 ValueError 발생, 제외 rfind 사용방법 동일

    # 특정 문자열로 시작 or 끝나는가?
    url = "http://www.naver.com"
    surl = "http://www.naver.com"
    ftp = "ftp://ftp.naver.com"

    print("url이 http://로 시작?", url.startswith("http://"))
    print("surl이 http://로 시작?", surl.startswith("http://"))
    # 검색시 시작 문자열을 여러 개 중 한개로 비교할 때
    print("ftp가 http://로 시작? or https://로 시작?",
            ftp.startswith(("http://", "https://")))
    print("url가 http://로 시작? or https://로 시작?",
          url.startswith(("http://", "https://")))
    print("url이 naver.com으로 끝나는가?", url.endswith("naver.com"))
    # startswitch, endswitch로 검색 범위를 제한할 수 있다.

    print("ftp의 0 ~ 20 영영이 ftp.으로 시작?", ftp.startswith("ftp.", 6, 20)) # 6 ~ 20 경계 영역이 ftp. 으로 시작?
...
def split_join_methods():
    ...
    분할과 합치기 메서드
    """
    s = "Ham and Chease and Breads and ketchup"
    print("원본:", s)
...
def check_method(): # 판별 관련 -> is 계열 메서드 -> bool
    ...
    print("Python 3". isalnum()) # 공백이 포함 -> alnum이 아님
    print(" \r\n\t".isspace()) # 공백문자만 포함?
    print("".isspace())
...
def align_methods():
    ...
    print("ZFILL", "1234567890".zfill(5)) # 확보한 자리는 최소 공간, 자리수 넘쳐도 내용은 잘리지 않음
...
def string_format():
    ...
    print(fmt % (10, '사과', 3))
    print("현재 이자율은 %.2f%%입니다" % 1.245) # %f 포맷은 부가적으로 소숫점 자리를 제한할 수 있다
    ...
    fmt = "{0}개의 {1} 중에서 {2}개를 먹었다" # 파라미터의 순서 명시
    print(fmt.format(10, "사과", 3))
    fmt = "{total}개의 {fruit} 중에서 {eat}개를 먹었다"
    print(fmt.format(eat=3, fruit="사과", total=10)) # 함수의 인자형태로 전달
    # 사전 객체 이용한 named parameter 연결: format_map 메서드
    data = {
        "total": 10, "fruit": "사과", "eat":3
    }
    print("{total}개의 {fruit} 중에서 {eat}개를 먹었다".format_map({"total": 10, "fruit": "사과", "eat":3}))
