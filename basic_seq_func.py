def using_range():
    ...
    print(seq[0], seq[1], seq[2]) # 정인덱스
    print(seq[-1], seq[-2], seq[-3]) # 역인덱스
...
def using_enumerate():
    """
    enumerate 함수 : 순차형에서 현재 아이템과 함꼐 내부 인덱스도 함께 필요할 떄 사용
    """
    i = 0
...
def using_zip():
    """
    zip 객체 : 여러 개의 순차자료형을 동시에 루프 시키는 객체
    """
    english = "Sun", "Mon", "Tue", "Wed"
    korean = "일요일", "월요일", "화요일", "수요일", "목요일"
    engkor = zip(english, korean) # 묶이는 조합의 길이는 짧은 쪽으로 맞춰진다
